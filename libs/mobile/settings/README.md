...
- **Watch-implies-add**: a watched, untracked tmdb-GUID item is added
  (`watchingViaPlex: true`, `traktId: null`). A movie → `completed`. A show is
  added with its **derived** status from the existing completion predicate —
  `completed` when every present episode doc is watched (`total > 0 && watched
=== total`), else `watching` (spec 0103, issue #277). Because episode docs are
  already created + mirrored earlier in the same pass, a fully-watched **ended**
  show is added `completed` on the **first** sync (not `watching` self-healing a
  sync later); a show with any unwatched (incl. scheduled/future) episode doc
  stays `watching`.
