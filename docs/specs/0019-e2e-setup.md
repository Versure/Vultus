...
- **F4 — watchlist → tap title → title-detail opens** (`seeded`) — **`test.fixme`
  ...
  entry's metadata. *Un-skips when spec 0016 lands.*
- **F5 — title-detail → change status (planned → watching) → watchlist reflects**
  ...
  Watchlist; the card now sits under the **Watching** section. *(Watchlist's own
  ...
  wants F5 partially green; the title-detail-originated change is the fixme part.)*
- **F6 — title-detail → remove → watchlist empty** (`seeded`) — **`test.fixme`
  (R1)**: remove from the detail page; Watchlist shows the empty state. *(Watchlist's
  own swipe/alert remove — `onDeleteConfirm`/`onDeleteItem` — is already runnable
  and MAY be the non-fixme part.)*
...
- **R7 — Java + chromium install in CI.** `emulators:exec` needs Java (already set
  ...
  the Nx cache keeps the rest cheap. **No new secret** is involved. *(Optional, non-
  critical: cache `~/.cache/ms-playwright` keyed on the installed Playwright version
  to avoid re-downloading chromium on every run — skip if it complicates the job.)*
